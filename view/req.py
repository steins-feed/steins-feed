...
def get_langs():
    ...
    res = [schema_feeds.Language[e] for e in flask.request.args.getlist('lang')]
...
def get_page():
    ...
    if get_timeunit() == get_timeunit(old=True):
        s = flask.request.args.get('page', default=None, type=str)
        if s:
...
def get_timeunit(old: bool=False):
    ...
    unit_name = flask.request.args.get(
        k,
        default = v,
        type=str,
    )
